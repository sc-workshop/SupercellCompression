	...
	namespace Compressor
	{
		struct ZstdProps
		{
			...
			/* Maximum allowed back-reference distance, expressed as power of 2.
			   ...
			   Note: Using a windowLog greater than ZSTD_WINDOWLOG_LIMIT_DEFAULT
					 requires explicitly allowing such size at streaming decompression stage. */
			int enableLongDistanceMatching = 0;
			...
			int nbWorkers = static_cast<int>(std::thread::hardware_concurrency());
		...
		class Zstd : public CompressionInterface
		{
		public:
			Zstd(ZstdProps& props);
			~Zstd();
		...
		private:
			...
			// -- Stream Buffer --
			const size_t Input_Buffer_Size = ZSTD_CStreamInSize();
